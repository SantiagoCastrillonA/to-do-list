document.addEventListener('DOMContentLoaded', () => {
    ...
    const clearCompletedBtn = document.querySelector('.todo-footer span:last-child');
    ...
    function addTodoToDOM(text, completed) {
        ...
        todoItem.innerHTML = `
            ...
            <span class="todo-text" style="${completed ? 'text-decoration: line-through; color: #999;' : ''}">${text}</span>
    ...
    function setupTodoItemEvents(todoItem) {
        ...
        checkbox.addEventListener('click', () => {
            ...
            todoText.style.color = checkbox.classList.contains('checked') ? '#999' : '';
            updateItemsCount();
            saveTodos();
        });
    ...
    function updateItemsCount() {
        const activeItems = document.querySelectorAll('.todo-item:not(.hidden) .todo-checkbox:not(.checked)').length;
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            todos.forEach(todo => {
                const isCompleted = todo.querySelector('.todo-checkbox').classList.contains('checked');
